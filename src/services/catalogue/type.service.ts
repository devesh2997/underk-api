...
export class TypeService {
    static get = async (typeInfo: any): Promise<Type> | never => {
        let err, type: Type
    ...
    static delete = async (typeInfo: any): Promise<Type> | never => {
    ...
    static create = async (typeInfo: any): Promise<Type> | never =>{
        ...
        [err, type] = await TO(Type.insert(type))
        if(err){
            TE(err)
        }
